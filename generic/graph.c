...
static int GraphNodesGetNodes(Graph* graphPtr, Tcl_Interp* interp, int objc,
        Tcl_Obj* const objv[])
{
    static char* opts[] = {
            "-labels",
    ...
    if (objc > 0) {
        if (Tcl_GetIndexFromObj(interp, objv[0], opts, "option", 0,
                &optIdx) != TCL_OK) {
            return TCL_ERROR;
        }
    }
...
static int GraphNodesAddNodes(Graph* graphPtr, Tcl_Interp* interp, int objc,
        Tcl_Obj* const objv[])
{
    /* Validation first. No node is added unless all nodes are valid */
    for (int j = 0; j < objc; j++) {
        const char* nName = Tcl_GetString(objv[j]);
        Node* nodePtr = Graphs_ValidateNodeCommand(graphPtr->statePtr, interp,
                nName);
        if (nodePtr == NULL) {
            return TCL_ERROR;
        }
...
static int GraphNodesDeleteNodes(Graph* graphPtr, Tcl_Interp* interp, int objc,
        Tcl_Obj* const objv[])
{
    return TCL_OK;
}

static int GraphCmdNodes(Graph* graphPtr, Tcl_Interp* interp, int objc,
        Tcl_Obj* const objv[])
{
    int cmdIdx;
...
static int GraphCmdConfigure(Graph* graphPtr, Tcl_Interp* interp, int objc,
        Tcl_Obj* const objv[])
{
    int i, optIdx;
    ...
    for (i = 0; i < objc; i += 2) {
        ...
        default: {
            Tcl_SetObjResult(interp,
                    Tcl_NewStringObj("Wrong options in graph configure", -1));
            return TCL_ERROR;
        }
        }
    }
...
static int GraphCmdCget(Graph* graphPtr, Tcl_Interp* interp, int objc,
        Tcl_Obj* const objv[])
{
    int optIdx;
    char* opts[] = {
    ...
    if (Tcl_GetIndexFromObj(interp, objv[0], opts, "option", 1,
            &optIdx) != TCL_OK) {
        return TCL_ERROR;
    }
...
static int GraphCmdDelete(Graph* graphPtr, Tcl_Interp *interp, int objc,
        Tcl_Obj * const objv[])
{
    int optIdx;
    static const char *opts[] = {
...
static int GraphInfoEdges(Graph* graphPtr, Tcl_Interp* interp, int objc,
        Tcl_Obj* const objv[])
{
    return TCL_OK;
}

static int GraphInfoDelta(Graph* graphPtr, DeltaT deltaType, Tcl_Interp* interp,
        int objc, Tcl_Obj* const objv[])
{
    ...
    while (entry != NULL) {
        ...
        Graphs_GetDelta(nodePtr, graphPtr, deltaType, lblFilt, interp,
                &resultObj);
...
static int GraphCmdInfo(Graph* graphPtr, Tcl_Interp* interp, int objc,
        Tcl_Obj* const objv[])
{
    int cmdIdx;
    static const char* subCmds[] = {
    ...
    if (Tcl_GetIndexFromObj(interp, objv[0], subCmds, "option", TCL_EXACT,
            &cmdIdx) != TCL_OK) {
        return TCL_ERROR;
    }
...
static int GraphSubCmd(Graph* graphPtr, Tcl_Obj* cmd, Tcl_Interp* interp,
        int objc, Tcl_Obj* const objv[])
{
    int cmdIdx;

    if (Tcl_GetIndexFromObj(interp, cmd, graphSubCommands, "method", 0,
            &cmdIdx) != TCL_OK) {
        return TCL_ERROR;
    }
...
static int Graph_GraphSubCmd(ClientData clientData, Tcl_Interp* interp,
        int objc, Tcl_Obj* const objv[])
{
...
static void GraphDeleteCmd(ClientData clientData)
{
    ...
    Tcl_HashEntry* entryPtr = Tcl_FindHashEntry(&g->statePtr->graphs,
            g->cmdName);
...
int Graph_GraphCmd(ClientData clientData, Tcl_Interp *interp, int objc,
        Tcl_Obj * const objv[])
{
    ...
    if (Tcl_GetIndexFromObj(interp, objv[1], graphSubCommands, "method", 0,
            &cmdIdx) != TCL_OK) {
        return TCL_ERROR;
    }
    ...
    case GraphCreateIx: {
        ...
        if (Tcl_StringMatch(Tcl_GetString(objv[1]), "new")) {
            sprintf(graphPtr->cmdName, "::tclgraphs::Graph%d",
                    gState->graphUid);
            gState->graphUid++;
            paramOffset = 2;
        }
        ...
        entryPtr = Tcl_CreateHashEntry(&gState->graphs, graphPtr->cmdName,
                &new);
        Tcl_SetHashValue(entryPtr, (ClientData )graphPtr);
        if (objc > paramOffset) {
            if (GraphCmdConfigure(graphPtr, interp, objc - paramOffset,
                    objv + paramOffset) != TCL_OK) {
                GraphDeleteCmd(graphPtr);
                return TCL_ERROR;
            }
        }

        graphPtr->commandTkn = Tcl_CreateObjCommand(interp, graphPtr->cmdName,
                Graph_GraphSubCmd, graphPtr, GraphDeleteCmd);
    ...
    graphPtr = Graphs_ValidateGraphCommand(gState, interp,
